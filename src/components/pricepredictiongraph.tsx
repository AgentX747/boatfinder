...
function pillStyle(v: number | null): React.CSSProperties {
  const base: React.CSSProperties = {
    ...
    borderRadius: 99, minWidth: 58, textAlign: "center",
  };
...
function BarTrack({ pct, color, width = 110 }: { pct: number; color: string; width?: number }) {
  return (
    <div style={{ width, height: 4, background: BLUE[50], borderRadius: 99, overflow: "hidden" }}>
...
export default function FerryForecastDashboard() {
  ...
  for (let i = 1; i < all.length; i++) {
    const prev = all[i - 1].val, cur = all[i].val;
    all[i].mom = prev ? (cur - prev) / prev * 100 : null;
  }

  const peak       = Math.max(...all.map(d => d.val), 1);
  const fcOnly     = all.filter(d => d.isFc);
  const maxFcVal   = Math.max(...fcOnly.map(d => d.val), 1);
  const shockPoint = fcOnly.find(d => d.oil_shock);
  ...
  const yData  = years.map((y, i) => {
    ...
    return { year: y, yoy: prev ? (cur - prev) / prev * 100 : null, isFc: yearMap[y].isFc };
  });
  const yDataFiltered = yData.filter(y => y.yoy != null);
  const maxYoy        = Math.max(...yDataFiltered.map(y => Math.abs(y.yoy!)), 1);
  ...
  const rowBase: React.CSSProperties = {
    ...
    padding: "11px 0", borderBottom: `0.5px solid ${BLUE[50]}`,
  };
  ...
  return (
    <div style={{ fontFamily: "Inter, -apple-system, BlinkMacSystemFont, sans-serif", padding: "1.5rem 0", maxWidth: 680, color: BLUE[900] }}>
        ...
        {TABS.map(t => (
          <button
            ...
            style={{
              ...
              borderRadius: 8, border: `0.5px solid ${BLUE[100]}`,
      ...
      {tab === "monthly" && (
        <>
          {shockPoint && (
            <Alert warn text={`Oil shock: ${shockPoint.month} ${shockPoint.year} — Iran/Hormuz modelled as +36% Brent spike. MARINA approved 20% fare hike Mar 16 2026.`} />
          )}
          <Card label="Forecast — % of peak & month-over-month change">
            {/* Legend */}
            <div style={{ display: "flex", gap: 16, fontSize: 11, color: BLUE[600], marginBottom: 14, alignItems: "center" }}>
              <span style={{ display: "flex", alignItems: "center", gap: 5 }}>
                <span style={{ width: 8, height: 8, borderRadius: "50%", background: BLUE[400], display: "inline-block" }} />
                Volume bar
              </span>
              <span style={{ display: "flex", alignItems: "center", gap: 5 }}>
                <span style={{ width: 28, height: 8, borderRadius: 3, background: "#EAF3DE", display: "inline-block" }} />
                Rising MoM
              </span>
              <span style={{ display: "flex", alignItems: "center", gap: 5 }}>
                <span style={{ width: 28, height: 8, borderRadius: 3, background: "#FCEBEB", display: "inline-block" }} />
                Falling MoM
              </span>
            </div>
              ...
              return (
                <div key={`${d.month}-${d.year}`} style={{ ...rowBase }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                    <span style={{ fontSize: 13, color: BLUE[900], fontWeight: 500, minWidth: 72 }}>
                      {d.month} {d.year}
                      {d.oil_shock && <span style={{ fontSize: 10, color: "#A32D2D", marginLeft: 4 }}>OIL</span>}
                    </span>
                    <BarTrack pct={barW} color={BLUE[400]} width={110} />
                  </div>
                  <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                    <span style={pillStyle(d.mom ?? null)}>{fmt(d.mom ?? null)}</span>
                    <span style={{ fontSize: 13, fontWeight: 500, color: BLUE[900], minWidth: 42, textAlign: "right" }}>
                      {peakPct.toFixed(1)}%
                    </span>
                  </div>
                </div>
        ...
        <Card label="Year-over-year passenger growth">
          {yDataFiltered.map(y => (
            <div key={y.year} style={{ ...rowBase }}>
              <span style={{ fontSize: 13, color: BLUE[900], fontWeight: 500 }}>
                {y.isFc ? `${y.year} (forecast)` : String(y.year)}
              </span>
              <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                <BarTrack pct={Math.abs(y.yoy!) / maxYoy * 100} color={y.yoy! >= 0 ? "#639922" : "#E24B4A"} width={100} />
            ...
            {mns.map(mn => {
              ...
              return (
                <div key={mn} style={{ ...rowBase }}>
                  ...
                  <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                    <BarTrack pct={pct != null ? Math.abs(pct) / maxSeaPct * 100 : 0} color={pct != null && pct >= 0 ? "#639922" : "#E24B4A"} width={100} />
