...
export default function BookBoat() {
  ...
  // ── Fetch boat details + session on mount ───────────────────────────────────
  useEffect(() => {
  ...
  // ── Fetch slot capacity whenever tripDate changes ───────────────────────────
  useEffect(() => {
    if (!tripDate || !boatID) return;
    ...
    async function fetchSlotCapacity() {
      ...
      try {
        const res = await apiFetch(
          `https://boatfinder.onrender.com/user/slot-capacity/${boatID}?tripDate=${tripDate}`,
          { method: "GET", credentials: "include" }
        );
        if (res.ok) {
        ...
        console.error("Failed to fetch slot capacity", err);
      } finally {
  ...
  function validateBooking(): boolean {
    ...
    // Guard against booking a full slot (extra client-side check)
    const cap = slotCapacity[selectedSchedule.departureTime];
    if (cap && cap.remaining === 0) {
      alert(`The ${selectedSchedule.departureTime} slot is fully booked. Please select another slot.`);
      setSelectedSchedule(null);
      return false;
    }
                ...
                (() => {
                  ...
                  return (
                    <>
                      {/* Loading skeleton */}
                      {capacityLoading && (
                        <p className="text-xs text-blue-500 italic mb-1">
                      ...
                      {available.map((slot, index) => {
                        ...
                        const cap       = slotCapacity[slot.departureTime];
                        const remaining = cap?.remaining ?? null;   // null = still loading
                        ...
                        return (
                          <button
                            ...
                            disabled={isFull}
                            onClick={() => {
                              if (!isFull) setSelectedSchedule(slot);
                            }}
                          ...
                          >
                            <div className="flex items-center justify-between gap-4">
                              ...
                              <div className="flex items-center gap-2 flex-shrink-0">
                                {/* Seat badge */}
                                {!capacityLoading && remaining !== null && (
                                  <span
