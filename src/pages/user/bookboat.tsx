...
export default function BookBoat() {
  ...
  // ── On mount: session + boat details + weather ────────────────────────────
  useEffect(() => {
      ...
      if (response.ok) {
        ...
        if (response.status === 422) {
          ...
          setNoGoModal({
            open: true,
            tripDate,
            shortSummary: match?.short_summary || match?.summary || data.message,
            fullSummary:  match?.summary || data.message,
          });
          return;
        }
        ...
        <div className="space-y-8">
          ...
          <div>
            ...
            <div className={`p-5 rounded-lg border-2 transition-colors ${
              ...
              {tripDate && !weatherLoaded && !weatherError && (
                <p className="text-xs text-blue-500 mt-2 animate-pulse font-medium">
                  ⏳ Checking weather safety data…
                </p>
              )}
