      ...
      <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-xl max-h-[90vh] flex flex-col overflow-hidden border border-slate-200">
        <div className={`bg-gradient-to-r ${headerGradient} px-5 sm:px-6 py-4 sm:py-5 flex items-start gap-3`}>
          ...
          <div className="flex items-center gap-1 flex-shrink-0">
            <button onClick={fetchData} disabled={loading} className="p-1.5 rounded-lg hover:bg-white/20 transition-colors text-white disabled:opacity-50" title="Refresh">
              <RefreshCw className={`w-4 h-4 ${loading ? "animate-spin" : ""}`} />
            </button>
            <button onClick={onClose} className="p-1.5 rounded-lg hover:bg-white/20 transition-colors text-white" title="Close">
              <X className="w-4 h-4" />
            </button>
          </div>
        </div>
...
export default function UserDashboard() {
  ...
  const [refundDetails, setRefundDetails] = useState({ ticketCode: "", operatorId: "", shortMessage: "", fileAttachment: null as File | null })
  ...
  const [cancelledTripDetails, setCancelledTripDetails] = useState<{
    boatName: string; tripDate: string; ticketcode: string; reason: string
  }[]>([])
  ...
  async function submitRefund() {
    try {
      if (!refundDetails.ticketCode || !refundDetails.operatorId || !refundDetails.shortMessage) { alert("Please fill in all required fields."); return }
      ...
      if (!res.ok) {
        ...
        if (res.status === 404 || (res.status === 401 && err.message === "Operator not found")) { alert("Operator not found. Please check the operator ID and try again."); return }
  ...
  async function submitTicket() {
    try {
      const res = await apiFetch("https://boatfinder.onrender.com/user/submitticket", { method: "POST", credentials: "include", headers: { "Content-Type": "application/json" }, body: JSON.stringify(ticketDetails) })
  ...
  useEffect(() => {
    async function fetchAll() {
      ...
      } catch (err) { console.error("Failed to fetch session", err); navigate("/login"); return }
    ...
    setBoatsLoading(false)
      ...
      async function getPendingBookings(): Promise<any[]> {
        try {
          ...
          return data
        } catch (err) { console.error("Failed to fetch pending bookings", err); return [] }
      }

      async function getAcceptedBookings(): Promise<any[]> {
        try {
          ...
          return data
        } catch (err) { console.error("Failed to fetch accepted bookings", err); return [] }
      }
    ...
    switch (activeTab) {
      case "searchrouteandtimeslot": {
        ...
        const matchingBoats = activeBoats.filter((boat: any) => {
          ...
          return (!searchBoat.routeFrom || from === searchBoat.routeFrom) && (!searchBoat.routeTo || to === searchBoat.routeTo)
        })
        ...
        matchingBoats.forEach((boat: any) => {
          ...
          sched.forEach((s: any) => {
            ...
            if (dep && !allSlots.some(x => x.departureTime === dep && x.arrivalTime === arr)) allSlots.push({ departureTime: dep, arrivalTime: arr })
          })
        })
        const depTimes = [...new Set(allSlots.map(s => s.departureTime))]
        const arrTimes = [...new Set(allSlots.filter(s => !searchBoat.departureTime || s.departureTime === searchBoat.departureTime).map(s => s.arrivalTime))]
        const fallbackTimes = ["12:00 AM","1:00 AM","2:00 AM","3:00 AM","4:00 AM","5:00 AM","6:00 AM","7:00 AM","8:00 AM","9:00 AM","10:00 AM","11:00 AM","12:00 PM","1:00 PM","2:00 PM","3:00 PM","4:00 PM","5:00 PM","6:00 PM","7:00 PM","8:00 PM","9:00 PM","10:00 PM","11:00 PM"]
            ...
            <div className="max-w-6xl mx-auto">
                  ...
                  <div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                      <div>
                        ...
                        <select name="routeFrom" className="w-full px-4 py-2 border border-blue-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 bg-white text-sm sm:text-base" value={searchBoat.routeFrom}
                          onChange={(e) => { setSearchBoat(prev => ({ ...prev, routeFrom: e.target.value, routeTo: "", departureTime: "", arrivalTime: "" })); setSearchResults([]); setHasSearched(false) }}>
                          <option value="">Select departure port</option>
                          {DEPARTURE_PORTS.map(port => <option key={port} value={port}>{port}</option>)}
                        </select>
                      </div>
                      <div>
                        <label className="block text-sm font-medium text-gray-700 mb-2">Destination</label>
                        <select name="routeTo" disabled={!searchBoat.routeFrom} className="w-full px-4 py-2 border border-blue-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 bg-white text-sm sm:text-base disabled:bg-gray-100 disabled:text-gray-400 disabled:cursor-not-allowed" value={searchBoat.routeTo}
                          onChange={(e) => { setSearchBoat(prev => ({ ...prev, routeTo: e.target.value, departureTime: "", arrivalTime: "" })); setSearchResults([]); setHasSearched(false) }}>
                          <option value="">{searchBoat.routeFrom ? "Select destination" : "Select origin first"}</option>
                          {(ROUTE_MAP[searchBoat.routeFrom] ?? []).map(dest => <option key={dest} value={dest}>{dest}</option>)}
                        </select>
                      </div>
                    </div>
                  </div>
                  <div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
                      <div>
                        ...
                        <select name="departureTime" className="w-full px-4 py-2 border border-blue-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 bg-white text-sm sm:text-base" value={searchBoat.departureTime}
                          onChange={(e) => setSearchBoat(prev => ({ ...prev, departureTime: e.target.value, arrivalTime: "" }))}>
                          <option value="">Select departure time</option>
                          {(depTimes.length > 0 ? depTimes : fallbackTimes).map((t, i) => <option key={`dep-${i}`} value={t}>{t}</option>)}
                        </select>
                      </div>
                      <div>
                        <label className="block text-sm font-medium text-gray-700 mb-2">Arrival Time</label>
                        <select name="arrivalTime" disabled={!searchBoat.departureTime} className="w-full px-4 py-2 border border-blue-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 bg-white text-sm sm:text-base disabled:bg-gray-100 disabled:text-gray-400 disabled:cursor-not-allowed" value={searchBoat.arrivalTime} onChange={handleSearchChange}>
                          <option value="">{searchBoat.departureTime ? "Select arrival time" : "Select departure first"}</option>
                          {(arrTimes.length > 0 ? arrTimes : fallbackTimes).map((t, i) => <option key={`arr-${i}`} value={t}>{t}</option>)}
                        </select>
                      </div>
                    </div>
                  </div>
                  <div className="flex flex-wrap gap-3">
                    <button className="px-8 py-3 bg-blue-600 text-white font-semibold rounded-lg hover:bg-blue-700 transition-colors shadow-md text-sm sm:text-base" onClick={handleSearchBoat}>Search</button>
                    {hasSearched && (
                      <button className="px-8 py-3 bg-white text-blue-600 font-semibold rounded-lg border-2 border-blue-300 hover:bg-blue-50 transition-colors shadow-sm text-sm sm:text-base flex items-center gap-2" onClick={handleClearSearch}>
              ...
              {noResults ? (
                <div className="bg-white rounded-lg shadow-lg p-10 flex flex-col items-center justify-center text-center gap-4">
                  <div className="w-16 h-16 rounded-full bg-blue-50 flex items-center justify-center"><SearchX className="w-8 h-8 text-blue-400" /></div>
                  <div>
                    ...
                    <table className="w-full min-w-[800px]">
                      <thead>
                        <tr className="bg-blue-600 text-white">
                          {["Boat ID","Operator Name","Boat Name","Vessel Type","Capacity","Ticket Price","Route From","Route To","Schedules","Actions"].map(h => (
                        ...
                        {tableBoats.map((boat: any) => (
                          <tr key={boat.boatId || boat.boat_id} className="border-b hover:bg-gray-50">
                            ...
                            <td className="px-4 sm:px-6 py-4 sm:py-5">
                              {(() => {
                                const sched = Array.isArray(boat.schedules) ? boat.schedules : []
                                if (sched.length === 0) return <span className="text-gray-400 italic text-xs">No schedules</span>
                                return (
                                  <div className="flex flex-col gap-1">
                                    {sched.map((s: any, i: number) => (
                                      <span key={i} className="inline-flex items-center gap-1 text-xs bg-blue-50 border border-blue-200 text-blue-700 rounded-md px-2 py-0.5 whitespace-nowrap">
        ...
        return (
          <main className="min-h-screen bg-gradient-to-br from-blue-50 to-blue-100 py-5 sm:py-8 px-3 sm:px-4 md:px-8">
            <div className="max-w-7xl mx-auto">
              ...
              <input type="text" value={ticketSearch} onChange={e => setTicketSearch(e.target.value)} placeholder="Search by ticket code or boat name..."
                ...
                {bookingsActiveTab === "history" && (
                  <>
                    ...
                    {filteredBookingHistory.length > 0 ? filteredBookingHistory.map(booking => (
                      <UserBookingHistoryCard key={booking.booking_id} booking_id={booking.booking_id} boatName={booking.boatName} bookDate={booking.bookDate} schedules={booking.schedules ?? null} bookPrice={booking.bookPrice} status={booking.status} boatstatus={booking.boatstatus} tripDate={booking.trip_date} ticketCode={booking.ticketcode} />
                    )) : <p className="text-sm text-gray-400">{ticketSearch ? `No history matching "${ticketSearch}"` : "No past bookings found."}</p>}
                  </>
                )}
              </div>
            </div>
                    ...
                    <div className="space-y-8 sm:space-y-10">
                      <div>
                        ...
                        {supportTickets.length === 0 ? <p className="text-gray-400 text-sm italic">No support tickets found.</p>
                          : <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">{supportTickets.map((t: any) => <SupportTicketCard key={t.ticket_id} ticketId={t.ticket_id} adminId={t.adminId ?? null} ticketSubject={t.ticketSubject} status={t.status} navigateTo={() => navigate(`/viewticket/${t.ticket_id}`)} />)}</div>}
                      </div>
                      ...
                      <div>
                        ...
                        {refundTickets.length === 0 ? <p className="text-gray-400 text-sm italic">No refund requests found.</p>
                          : <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">{refundTickets.map((r: any) => <RefundTicketCard key={r.request_id} requestId={String(r.request_id)} ticketCode={r.ticketcode ?? "N/A"} operatorId={String(r.operator_id)} operatorName={r.operatorName ?? "Unknown"} status={r.status ?? "pending"} navigateTo={() => navigate(`/viewrefund/${r.request_id}`)} />)}</div>}
                      </div>
                    </div>
                  )}
                      ...
                      <div className="space-y-3">
                        ...
                        <label className="flex flex-col items-center justify-center w-full h-32 sm:h-36 border-2 border-dashed border-blue-300 rounded-lg cursor-pointer bg-blue-50/40 hover:bg-blue-50 transition-colors group">
                          <div className="flex flex-col items-center gap-2 text-center px-4">
                            ...
                            {refundDetails.fileAttachment ? (<><p className="text-sm font-semibold text-blue-700">{refundDetails.fileAttachment.name}</p><p className="text-xs text-gray-400">{(refundDetails.fileAttachment.size / 1024).toFixed(1)} KB — click to replace</p></>) : (<><p className="text-sm font-semibold text-blue-600">Click to upload or drag &amp; drop</p><p className="text-xs text-gray-400">PNG, JPG, PDF up to 10MB</p></>)}
                          </div>
          ...
          <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-lg max-h-[90vh] flex flex-col overflow-hidden border border-red-200">
            <div className="bg-gradient-to-r from-red-600 to-red-700 px-5 sm:px-6 py-4 sm:py-5 flex items-start gap-3">
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2 mb-0.5">
                  ...
                  <h2 className="text-white font-bold text-base sm:text-lg">Your Ferry Trip{cancelledTripDetails.length > 1 ? "s Have" : " Has"} Been Cancelled</h2>
                </div>
                <p className="text-white/80 text-xs sm:text-sm">
                  Due to dangerous weather conditions, {cancelledTripDetails.length > 1 ? "these trips have" : "this trip has"} been automatically cancelled for your safety.
                </p>
              </div>
            ...
            <div className="flex-1 overflow-y-auto p-4 sm:p-5 space-y-3">
              <div className="flex items-start gap-3 p-3 rounded-xl bg-red-50 border border-red-200">
                ...
                <p className="text-xs text-red-800 leading-relaxed">
                  Sealegs SpotCast has classified these travel dates as <span className="font-bold">NO-GO</span> due to unsafe maritime conditions. Please contact the operator or use the refund button below for assistance.
                </p>
              </div>
              ...
              {cancelledTripDetails.map((trip, i) => (
                <div key={i} className="rounded-xl border border-red-200 bg-red-50 p-4">
                  ...
                  <div className="space-y-1.5">
                    ...
                    <div className="flex items-center justify-between gap-2">
                      <span className="text-xs text-red-500 font-semibold uppercase tracking-wide">Ticket Code</span>
                      <span className="text-sm font-mono font-semibold text-red-900">{trip.ticketcode}</span>
                    </div>
                    <div className="flex items-center justify-between gap-2">
                      <span className="text-xs text-red-500 font-semibold uppercase tracking-wide">Reason</span>
                      <span className="text-xs text-red-700 text-right">{trip.reason}</span>
                    </div>
                  </div>
                </div>
              ))}
            ...
            <div className="px-5 py-3 border-t border-red-100 bg-red-50 flex items-center justify-between gap-3">
              ...
              <div className="flex gap-2">
                <button
                  onClick={() => { setCancelWarningOpen(false); setActiveTab("ticketsupport") }}
        ...
        <div className="fixed inset-0 backdrop-blur-sm bg-black/30 flex items-center justify-center p-3 sm:p-4 z-50">
          <div className="bg-gradient-to-br from-blue-50 to-indigo-50 rounded-xl shadow-2xl max-w-md w-full p-5 sm:p-8 relative border border-blue-200 mx-3 sm:mx-0">
            ...
            <div className="flex gap-3">
              <button onClick={modalConfig.onConfirm} disabled={cancelling} className={`flex-1 px-4 py-2.5 rounded-lg font-semibold transition-colors shadow-md text-sm sm:text-base disabled:opacity-60 ${modalConfig.confirmStyle}`}>
                {cancelling ? "Processing…" : modalConfig.confirmText}
              </button>
              <button onClick={closeModal} disabled={cancelling} className="flex-1 px-4 py-2.5 rounded-lg font-semibold border-2 border-blue-300 text-blue-700 hover:bg-blue-100 transition-colors text-sm sm:text-base disabled:opacity-60">
                Keep Booking
              </button>
            </div>
          </div>
